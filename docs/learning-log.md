...
| 2026-04-28 | Milestone 5: Database Planning | Learned how to design relational database tables, primary keys, foreign keys, status constraints, and sample seed data for ExpiryWise. | Created `database-design.md`, `schema.sql`, and `seed.sql`. |
| 2026-04-27 | Milestone 6: Database Setup | Learned how to connect an Express backend to PostgreSQL using `pg`, store database credentials in environment variables, test the database connection, and query seeded food item data from the database. | Created `db.js`, `.env.example`, `/api/db-test`, and updated `/api/food-items` to read from PostgreSQL. |